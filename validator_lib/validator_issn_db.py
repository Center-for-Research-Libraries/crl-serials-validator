...
class ValidatorIssnDb:
    ...
    def process_title_dicts(self, title_dicts, input_file):
        ...
        for title_dict in title_dicts:
            ...
            title_dict["issn_db_form_not_print"] = db_data_local[
                "issn_db_form_not_print"
            ]
            title_dict["issn_db_serial_type"] = db_data_local["issn_db_serial_type"]
            title_dict["issn_db_serial_type_not_periodical"] = db_data_local[
            ...
            title_dict["holdings_out_of_issn_db_date_range"] = db_data_local[
                "holdings_out_of_issn_db_date_range"
            ]
            title_dict["local_issn_does_not_match_issn_db"] = db_data_local[
                "issn_mismatch"
            ]
            title_dict["wc_issn_does_not_match_issn_db"] = db_data_wc["issn_mismatch"]
            if (
                title_dict["local_issn_does_not_match_issn_db"]
                and title_dict["wc_issn_does_not_match_issn_db"]
            ):
    ...
    def get_issn_db_data(self, issn, year_1, year_2, issn_source="local"):
        ...
        if (
            not year_1
            or not db_data["issn_db_year_1"].isdigit()
            or not db_data["issn_db_year_2"].isdigit()
        ):
    ...
    def _check_valid_issn_marc(self, marc, year_1, year_2):
        ...
        if issn_data.form in self.valid_forms:
            if issn_data.serial_type in self.valid_serial_types:
                if self.check_holdings_dates_against_issn_dates(
                    issn_data, year_1, year_2
                ):
                    return issn_data
        return None
