...
class ReviewWorkbookPrinter:
    ...
    def make_error_counts_output(self, inst):
        blank_line = ["", ""]
        output = [[inst, ""], blank_line, ["errors", "count"]]
    ...
    def make_disqualifying_error_counts_output(self, inst):
        blank_line = ["", ""]
        output = [[inst, ""], blank_line, ["errors", "count"]]
        for disqualifying_error_tuple in self.disqualifying_error_counter[
            inst
        ].most_common():
            ...
            for cat in self.checklist_cats:
                if "issn_db" in cat and not ISSN_DB_LOCATION:
                    ...
                    try:
                        ...
                        logging.error(
                            "Category {} not seen in checklist data.".format(cat)
                        )
    ...
    def make_workbooks(self):
        for inst in self.outputs:
            ...
            for_review_list, for_review_special_rows = self.make_error_worksheet(
                self.outputs[inst]["for_review"]
            )
            error_count_output = self.make_error_counts_output(inst)
            disqualifying_error_count_output = (
                self.make_disqualifying_error_counts_output(inst)
            )
            ...
            for_review_special_formats = [
                ...
                (
                    {"bold": True, "bg_color": "#FBE5D6", "font_size": "14"},
                    for_review_special_rows,
                ),
            ]
            ...
            if self.total_records[inst] >= 50000:
                ...
                error_file_location = validator_lib.utilities.get_unused_filename(
                    error_file_location
                )
    ...
    def print_headless_checklist(self, checklist_data, headless_output_filename):
        ...
        if good_output:
            good_headless_output_filename = headless_output_filename.replace(
                "review", "loading"
            )
            fout_good = open(
                good_headless_output_filename, "w", encoding="utf8", newline=""
            )
        ...
        if bad_output:
            bad_headless_output_filename = headless_output_filename.replace(
                "for review", "failed"
            )
            fout_bad = open(
                bad_headless_output_filename, "w", encoding="utf8", newline=""
            )
