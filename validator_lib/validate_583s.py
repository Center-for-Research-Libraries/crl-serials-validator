...
class Line583Validator:
    ...
    def __init__(self) -> None:
        ...
        self.output_lines: List[List[str]] = []
        self.output_lines.append(header)

    def get_output_data(self) -> List[List[str]]:
        return self.output_lines.copy()

    def validate_583_lines_in_record(self, record: str, record_dict: dict) -> None:
        ...
        if not saw_committed_to_retain:
            ...
            record_dict["line_583_error_details"].append(
                "no_committed_to_retain_in_583"
            )
    ...
    def validate_583_line(self, line, record_dict):
        ...
        if not subfield_c:
            ...
            record_dict["line_583_error_details"].append(
                "Missing $c in 583: {}".format(line)
            )
        if not subfields_f:
            errors.append("no_583_subfield_f")
            record_dict["line_583_error_details"].append(
                "Missing $f in 583: {}".format(line)
            )
