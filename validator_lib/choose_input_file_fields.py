...
class InputFieldsChooser:
    marc_instructions = colored(
        'Enter MARC fields/subfields like "001" or "904a".', "green"
    )
    spreadsheet_instructions = colored(
        "Enter columns by number, starting with 1.", "green"
    )
        ...
        while True:
            ...
            for input_file in self.all_input_files:
                ...
                if "_AUTOGENERATED_FILE" in input_file:
                    number_color = "red"
                print(
                    "{}. {}".format(colored(str(file_number), number_color), input_file)
                )

            print("")
            cprint("What would you like to do?", "cyan")
            print(
                "Enter a {} to set input fields for that file.".format(
            ...
            if user_choice.lower().startswith("m"):
                ...
                try:
                    ...
                    if wanted_file.lower().endswith(
                        "mrk"
                    ) or wanted_file.lower().endswith("mrc"):
    ...
    def get_input_file_header(input_file):
        header_list = []
        header_list.append(
            "Selecting fields for {}".format(colored(input_file, "cyan"))
        )
        bar = "~~~~~~~~~~~~~~~~~~~~~"
    ...
    def get_file_fields(self, input_file, input_cats):
        ...
        while True:
            ...
            cprint("What would you like to do?", "cyan")
            print(
                "Enter the {} of a {}/{} field to toggle its value.".format(
                    colored("number", "yellow"),
                    colored("yes", "blue"),
                    colored("no", "red"),
                )
            ...
            print(
                "Enter the {} of another field to add, delete, or alter a value.".format(
                    colored("number", "yellow")
                )
            )
            ...
            if user_choice.lower().startswith("m"):
                ...
                and int(user_choice) <= cat_number
            ):
                cat_to_change = input_cats[int(user_choice) - 1]
                if cat_to_change in self.checkbox_cats:
                    if (
                        cat_to_change in self.validator_config.config[input_file]
                        and self.validator_config.config[input_file][cat_to_change]
                        == "1"
                    ):
                    ...
                    self.get_free_text_field_data(
                        cat_to_change, input_file, cat_body_list
                    )
    ...
    def get_free_text_field_data(self, cat_to_change, input_file, cat_body_list):
        ...
        print("\n".join(cat_body_list))
        print(
            "Altering field {}.".format(colored(self.cat_labels[cat_to_change]), "cyan")
        )
        bar = "~~~~~~~~~~~~~~~~"
        ...
        print(
            "Press {} to cancel and not make any changes.".format(
        ...
        if user_choice.lower().startswith("n"):
            ...
            checked_new_value = self.basic_field_validation(
                input_file, cat_to_change, new_value
            )
    ...
    def basic_field_validation(self, input_file, field_name, field_data):
        ...
        if not field_data:
        ...
        elif not input_file.lower().endswith("mrk") and not input_file.lower().endswith(
            "mrc"
        ):
            # Spreadsheets
            if not field_data.isdigit():
                field_validation_warning = (
                    "Field data {} is invalid. Input must be a digit.".format(
                        field_data
                    )
                )
        else:
            ...
            if len(field_data) < 3 or len(field_data) > 4:
                field_validation_warning = "Input {} is invalid length.".format(
                    field_data
                )
            elif subfield and not re.search(r"\w", subfield):
                field_validation_warning = (
                    "Input {} includes invalid subfield {}.".format(
                        field_data, subfield
                    )
                )
                ...
                field_validation_warning = (
                    "Input field {} must include subfield.".format(field)
                )
        if field_validation_warning:
            print(
                "{} {}".format(
                    colored("INVALID ENTRY:", "white", "on_red"),
                    field_validation_warning,
                )
            )
