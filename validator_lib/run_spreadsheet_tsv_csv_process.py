            ...
            try:
                with open(input_file_location, "r", encoding=my_encoding) as fin:
                    for line in fin.readlines():
                        pass
                print(
                    "Will use encoding {} for file.".format(
                ...
                pass
        cprint(
            "Can't find text encoding of input file. Please convert it to UTF-8 or Windows text format."
        )
        sys.exit()
        ...
        for row in iterator:
            ...
            for cat in self.input_cats:
                ...
                if "holdings_" in cat and cat != "holdings_id" and cat_data:
                    ...
                    regular_holdings_str, _, _ = remove_supplements_from_holdings(
                        str(cat_data)
                    )
                    regular_holdings_str, _, _ = remove_indexes_from_holdings(
                        regular_holdings_str
                    )
            ...
            holdings_start, holdings_end = get_first_last_year_from_regular_holdings(
                regular_holdings
            )
            if holdings_start:
