...
def remove_supplements_from_holdings(holdings):
    ...
    if "supp" in working_holdings.lower():
        start_terms = ["supp", "special suppl", "*supp", "statistical supp", "[suppl"]
...
def remove_indexes_from_holdings(holdings):
    ...
    if "ind" not in holdings.lower():
        if "author" in holdings.lower():
            ...
            holdings = re.sub(
                "(authors?(?:/[a-z]+)?)", "\\1 index", holdings, flags=re.I
            )
        else:
            return holdings, "", False
    if not re.search(r"ind(?:ex|xe|ic[ei]|eks|ec|\.)", holdings, flags=re.I):
        if not re.search(
            r"(?:subj(?:ect)?|cum(?:ulative)?|auth(?:or)?|master)[\. ]*ind", holdings
        ):
    ...
    # v.39:1991+index
    m = re.search(
        r"( *(?:and|&|\+|incl\.?) *([^;,]*index[^;,]*))", holdings, flags=re.I
    )
    if m:
        replacement_string = m.group(1)
        y = re.search(
            r"[^(]+(\([^)]*(?:1[6789]\d\d|20[012]\d)?[^)]*\) *$)", replacement_string
        )
        if y:
    ...
    # 2008, index
    if re.search(
        r"^\d\d\d\d(?: *[/-] *\d\d(?:\d\d)?)? *[:,\[(] *ind", holdings, flags=re.I
    ):
        return "", original_holdings, True
...
def volume_transform(holdings_segment):
    ...
    if "jahrg" in holdings_segment.lower():
        ...
        if re.search(
            r"jahrg\.? *(?:1[6-9]\d\d|20[01]\d)", holdings_segment, flags=re.I
        ):
            holdings_segment = re.sub(r"jahrg\.? *", "", holdings_segment, flags=re.I)
        else:
            if re.search(r"\b(?:t|bd)\.? *\d", holdings_segment, flags=re.I):
                holdings_segment = re.sub(
                    r"(\b)(?:t|bd)\.? *(\d)", "\\1no.\\2", holdings_segment, flags=re.I
                )
            # 3. Jahrg., 1. T. (1925)
            holdings_segment = re.sub(
                r", (\d\d?\d?)\.? (?:t|bd)\.?", r", no.\1", holdings_segment, flags=re.I
            )
    ...
    if re.search(r"\b[Tt]\.? *\d", holdings_segment) and re.search(
        r"\blivr?e?\.? *\d", holdings_segment
    ):
        holdings_segment = re.sub(r"(\b)livr?e?\.? *", "\\1no.", holdings_segment)
    # "t.86:kn.1:1993" -> "kn."/"knj."/"kniga" to represent number
    if re.search(r"\b[TtGg]\.? *\d", holdings_segment) and re.search(
        r"\bknj?(?:iga)?\.? *\d", holdings_segment
    ):
    ...
    holdings_segment = re.sub(
        r"(\d+)[e\.]+ ann[ée]e", "v.\\1", holdings_segment, flags=re.I
    )

    # "annee23" is a relatively common sort of error
    holdings_segment = re.sub(
        r"ann[ée]e(\d)", "annee \\1", holdings_segment, flags=re.I
    )
    ...
    for volume_word in vol_words:
        holdings_segment = re.sub(
            r"(\b){}(\d)".format(volume_word), "\\1v.\\2", holdings_segment, flags=re.I
        )
    ...
    # ed. 24
    holdings_segment = re.sub(
        r"(\b)[Ee]d\.? *(\d{1,3}(\b))", "\\1v.\\2\\3", holdings_segment
    )

    # "14th ed."
    holdings_segment = re.sub(
        r"(\d+) *(?:st|nd|rd|th|er|re|e)\.? ed(?:ition)?\.* *",
        "v.\\1 ",
        holdings_segment,
    )
    ...
    # "4th year (1998)"
    holdings_segment = re.sub(
        r"(\d+){} y(?:ea)?r\.?".format(ordinals_regex),
        "v.\\1",
        holdings_segment,
        flags=re.I,
    )
    ...
    holdings_segment = re.sub(
        r"year (\d{1,3})(\b)", "v.\\1\\2", holdings_segment, flags=re.I
    )
    ...
    holdings_segment = re.sub(
        r"v\.(\d+), no\.? *(\d)", "v.\\1 no.\\2", holdings_segment
    )

    # "section" or an abbreviation used as number
    if "no." not in holdings_segment and "sec" in holdings_segment.lower():
        holdings_segment = re.sub(
            r"sect?(?:ion)?\.? *(\d{1,3}\b)", r"no.\1", holdings_segment, flags=re.I
        )

    # something like "17. Jahr (2004)" will be "17. v. (2004)" and will wind up as "v.17-v.2004"
    holdings_segment = re.sub(
        r"(\b)(\d{1,3})\.? *v\. *\(", "\\1v.\\2 (", holdings_segment
    )
    if "v." not in holdings_segment and re.search(
        r"\(\d\d?\d?(?:st|nd|rd|th|e|me)", holdings_segment
    ):
        holdings_segment = re.sub(
            r"\((\d\d?\d?)(?:st|nd|rd|th|e|me)\.? *", "(v.\\1", holdings_segment
        )

    # "203. Bd. (Jahrg. 1994)-206. Bd. (Jahrg. 1997)" now will be "v.203 (v. 1994)-v.206 (v. 1997)"

    holdings_segment = re.sub(
        r"(\b)v\. *(\d{1,3}) *\( *v\. *(\d\d\d\d)(\b)",
        "\\1v.\\2 (\\3\\4",
        holdings_segment,
    )
    ...
    holdings_segment = re.sub(
        r"(\b)v\. *(\d\d\d\d(?: *- *(?:v\.)? *\d\d\d\d)?) *(\d\d\d\d(?: *[/-] *\d\d\d?\d?)?)(\b)",
        "\\1v.\\2 (\\3)\\4",
        holdings_segment,
    )
    ...
    if not re.search(r"\([^)]*(?:1[6-9]\d\d|20[01]\d)", holdings_segment):
        holdings_segment = re.sub(
            r"(\b)v\. *((?:1[6-9]\d\d|20[01]\d))", "\\1\\2", holdings_segment
        )
    ...
    holdings_segment = re.sub(
        r"(\b)v\. *(\d+) *: *v\. *(\d+)(\b)", "\\1v.\\2:no.\\3\\4", holdings_segment
    )
    ...
    if re.search(r"^ *\d+{}[\s,]+v\. *\d".format(ordinals_regex), holdings_segment):
        ...
        for segment in m:
            ...
            updated_segment = re.sub(
                r"(\d+){}".format(ordinals_regex), "v.\\1", updated_segment, flags=re.I
            )
    ...
    holdings_segment = re.sub(
        r"\((v\.\d+ *- *\d+), *({})".format(years_regex), r"\(\1 \2", holdings_segment
    )
