...
class IssuesChooser:
        ...
        while True:
            ...
            for issue in self.validator_config.issue_categories:
                ...
                if "issn_db" in issue and not ISSN_DB_LOCATION:
                    ...
                    self.validator_config.config["disqualifying_issues"][issue] is True
                ):
                ...
                print(
                    "{}{}\t{}".format(
                        colored(str(issue_no).ljust(4), number_color),
                        issue.ljust(35),
                        print_symbol.ljust(4),
                    ),
                    end=line_end,
                )
            ...
            print("")
            print(
                "{} to toggle the option between yes and no.".format(
                    colored("enter a number", "yellow")
                )
            )
            print(
                "{}. Reset all values to the defaults.".format(colored("d", "yellow"))
            )
            print(
                "{}. Visit the Validator issues glossary in a web browser.".format(
                    colored("g", "yellow")
                )
            )
    ...
    def ok_clicked(self):
        for issue in self.validator_config.config["disqualifying_issues"]:
            ...
            self.validator_config.config["disqualifying_issues"][issue] = bool(
                issue_state
            )
        self.validator_config.write_validator_config_file()
        self.cancelled()
