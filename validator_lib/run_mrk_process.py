...
from validator_lib.supplements_and_indexes_functions import (
    remove_supplements_from_holdings,
    remove_indexes_from_holdings,
)
...
class MrkProcessRunner:
    ...
    def __init__(self, input_file, input_fields):
        ...
        self.input_file_location = os.path.join(VALIDATOR_INPUT_FOLDER, self.input_file)
    ...
    def get_data_from_marc(self):
        ...
        for record in mfr:
            ...
            if seqnum % 5000 == 0:
                logging.info(
                    "   ...reached record {} in {}".format(seqnum, self.input_file)
                )
    ...
    def get_data_from_record(self, record, seqnum):
        self.errors_this_record = []
        mf = MarcFields(
            record, log_warnings=True, debug_info="from {}".format(self.input_file)
        )
    ...
    def validate_line(self, marc_line, record_dict):
        ...
        for i in range(1, len(line_list)):
            if not re.search(r"^\w", line_list[i]):
                msg = "Incomplete, Dangling, or Illegal Subfield: {}".format(marc_line)
    ...
    def get_holdings_from_marc(self, record, record_dict):
        ...
        if "863" in self.input_fields and self.input_fields["863"]:
            ...
            for output_string in c.output_strings:
                if (
                    "supp" not in output_string.lower()
                    and "ind" not in output_string.lower()
                ):
        ...
        first_holdings_year, last_holdings_year = (
            get_first_last_year_from_regular_holdings(regular_holdings_list)
        )
