...
class BulkConfig(ValidatorConfig):
    ...
    def add_new_data_to_config(self):
        ...
        for input_field in self.input_fields:
            self.config["programs"][self.program_name]["input_fields"][input_field] = (
                self.input_fields[input_field]
            )
        for disqualifying_issue in self.disqualifying_issues:
            self.config["programs"][self.program_name]["disqualifying_issues"][
                disqualifying_issue
            ] = self.disqualifying_issues[disqualifying_issue]
...
def check_if_program_done_already(program_name, validator_config):
    for program in validator_config.config["programs"]:
        if program and program.lower() == program_name.lower():
            question = (
                "Data for {} found in configurations file. Overwrite? (y/n)".format(
                    program_name
                )
            )
    ...
    if program_name.lower() in validator_config.names_associated_to_programs_map:
        associated_program = validator_config.names_associated_to_programs_map[
            program_name.lower()
        ]
        print(
            "Name {} associated with program {} in config file. Cannot continue with removing this association.".format(
                program_name, associated_program
            )
        )
        ...
        if get_yes_no_response(question) is True:
            remove_me = set()
            try:
                for i, associated_name in enumerate(
                    validator_config.config["programs"][associated_program][
                        "associated_names"
                    ]
                ):
...
def get_program_name(validator_config):
    while True:
        program_name = get_varied_response(
            "Enter program identification:", blank_ok=False
        )
    ...
    while True:
        if not associated_names:
            if not get_yes_no_response(
                "Are there any other names associated with {}?".format(program_name)
            ):
                return
        ...
        if associated_name:
            if associated_name.lower() in validator_config.config["programs"]:
                print(
                    "Name {} found as program name in configuration file. Skipping.".format(
                        associated_name
                    )
                )
            elif (
                associated_name.lower()
                in validator_config.names_associated_to_programs_map
            ):
                associated_program = validator_config.names_associated_to_programs_map[
                    associated_name.lower()
                ]
                print(
                    "Name {} found associated to program name {} in configuration file. Skipping.".format(
                        associated_name, associated_program
                    )
                )
            else:
...
def get_input_filetype(program_name, program_dict):
    while True:
        ...
        response = get_varied_response(
            "Choose 1 or 2.", valid_responses=valid_responses, blank_ok=False
        )
        if get_yes_no_response(
            "You chose {}, {}. Correct?".format(response, valid_responses[response])
        ):
    ...
    for input_field in input_fields_list:
        if input_field == "863":
            ...
            if input_field == "holdings_1" or input_field == "holdings_2":
                response = get_varied_response(
                    "other holdings field".format(input_field)
                )
            else:
...
def get_spreadsheet_input_fields(program_dict):
    ...
    for input_field in input_fields:
        if input_field == "header":
            ...
            response = get_varied_response(
                "Column for {}:".format(input_field), blank_ok=True
            )
            program_dict["input_fields"][input_field] = response
...
def choose_disqualifying_issues(program_name, program_dict, validator_config):
    while True:
        default_disqualifying_issues = (
            validator_config.get_default_disqualifying_issues()
        )
        if get_yes_no_response(
            "Use default disqualifying issues for {}?".format(program_name)
        ):
            program_dict["disqualifying_issues"] = dict(default_disqualifying_issues)
        else:
            print(
                "Choosing disqualifying issues. Please enter 1 for True or 0 for False, or return to accept the default."
            )
            valid_responses = {"0", "1"}
            ...
            for issue in default_disqualifying_issues:
                ...
                response = get_varied_response(
                    "{} [default {}]".format(issue, default_response),
...
def run_bulk_config():
    ...
    while True:
        add_response = get_yes_no_response(
            "Add data for {} to configuration file? (y/n)".format(program_name)
        )
        if add_response is True:
            print(
                "Adding data for program {} to configuration file.".format(program_name)
            )
