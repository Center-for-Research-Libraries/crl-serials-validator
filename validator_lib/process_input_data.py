...
from crl_lib.validation_utilities import (
    check_for_print_carrier_type,
    check_for_print_media_type,
)
...
class InputDataProcessor:
    ...
    def __init__(
        self, title_dicts, input_fields, disqualifying_issue_categories, jstor_titles
    ):
    ...
    def get_institution(self, title_dict):
        if not title_dict["institution"]:
            if title_dict["field_852a"]:
                ...
                title_dict["institution"] = (
                    validator_lib.utilities.get_abbrev_from_input_filename(
                        title_dict["filename"]
                    )
                )
    ...
    def assemble_errors_in_dict(self, title_dict):
        ...
        for marc_issue in self.marc_issues_to_check:
            if (
                marc_issue in title_dict["errors"]
                and marc_issue in self.disqualifying_issue_categories
            ):
                title_dict["invalid_record"] = "1"
        ...
        title_dict["disqualifying_error_category"] = "; ".join(
    ...
    def run_holdings_checks(title_dict):
        ...
        title_dict["completeness_words_in_holdings"] = (
            validator_lib.utilities.check_holdings_data_for_magic_words(
                title_dict["local_holdings"],
                title_dict["nonpublic_notes"],
                title_dict["public_notes"],
                "completeness",
            )
        )

        title_dict["binding_words_in_holdings"] = (
            validator_lib.utilities.check_holdings_data_for_magic_words(
                title_dict["local_holdings"],
                title_dict["nonpublic_notes"],
                title_dict["public_notes"],
        ...
        title_dict["nonprint_words_in_holdings"] = (
            validator_lib.utilities.check_holdings_data_for_magic_words(
                title_dict["local_holdings"],
                title_dict["nonpublic_notes"],
                title_dict["public_notes"],
        ...
        if title_dict["holdings_start"]:
            ...
            if start_between is False:
                if (
                    "/" in title_dict["wc_line_362"]
                    or "/" in title_dict["local_holdings"]
                ):
                    start_between = (
                        validator_lib.utilities.double_check_slash_start_year(
                            title_dict["start_including_362"],
            ...
            if end_between is False:
                if (
                    ...
                    end_between = validator_lib.utilities.double_check_slash_end_year(
                        title_dict["start_including_362"],
                        title_dict["wc_line_362"],
                        title_dict["holdings_start"],
                        title_dict["local_holdings"],
    ...
    def check_type_of_continuing_resource(self, title_dict):
        ...
        if (
            ...
            and title_dict["serial_type"] in self.valid_serial_types
        ):
