...
def get_unused_filename(file_location):
    ...
    while True:
        ...
        if new_file_number >= 999:
            raise Exception(
                "At least 1000 files with the base name {}. Runaway process?".format(
                    full_filename
                )
            )
...
def check_holdings_data_for_magic_words(
    holdings, holdings_nonpublic_notes, holdings_public_notes, search_type
):
    search_data = {
    ...
    all_holdings_segments_to_check = [
        holdings,
...
def get_earlier_of_slash_year(year, data_segment):
    ...
    m = re.search(
        r"({}) *[-/] *(?:{}|{})".format(year_regex, year, short_year), data_segment
    )
    if m:
